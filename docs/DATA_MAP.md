...
### 2.4 ~~Relay legs~~ → ~~Joint co-ownership~~ → โน้ตเวร (เปลี่ยนระบบแล้ว 2 รอบ) + ระบบชื่อเล่น

> ⚠️ **ล้าสมัย 2 ชั้น — อัปเดต 2026-07-14:** ระบบ "ไม้" (relay legs) ที่เคยแมปไว้ในตารางนี้**ถูกลบทิ้งทั้งหมดแล้ว** (commit `95753bb`, ยืนยัน `_relayEnabled` เหลือ 0 จุด) แทนที่ด้วย **"ต่อเวรแบบเจ้าของร่วม" (joint co-ownership)** ซึ่งพัฒนาไปถึง v2.0 — **แล้วตัวนั้นก็ถูกแทนที่อีกทีด้วย "โน้ตเวร" (shift notes ต่อคน)** (commit `162dabc` "feat(note): แทน 'เจ้าของร่วม' (joint) ด้วย 'โน้ตเวร' ต่อคน", ยืนยันแล้วว่า `_combinedJointMap`/joint-related code เหลือ 0 จุด ส่วน `_shiftNoteMap`/`_noteEdit`/`_noteView` พบ 16 จุดในโค้ดปัจจุบัน) หลักการของโน้ตเวรง่ายกว่าทั้งไม้และ joint มาก: เวรยังเป็นของคนเดียว (single owner) แค่แปะโน้ตอิสระได้ เลือก 🌐public (sync ทุกคนเห็น) หรือ 🔒private (เก็บเครื่องเดียว) — **ไม่มี** multi-owner, ไม่มี strike/ghost/chain เลย **ตารางด้านล่าง (relay leg record) เป็นข้อมูลประวัติศาสตร์ทั้งคู่ ไม่ตรงกับโค้ดปัจจุบันแล้ว** — โดเมนนี้ยังไม่ถูกแมปใหม่ตามระบบโน้ต (รอทำรอบหน้าถ้าต้องการ) ส่วนแถวชื่อเล่นด้านล่างไม่ได้รับผลกระทบ ยังตรงกับโค้ดปัจจุบัน
>
> **บทเรียนของการล้าสมัยซ้ำสองรอบนี้:** ระบบนี้เปลี่ยนเร็วมาก (หลาย commit/วัน จากหลาย session พร้อมกัน) — ก่อนเชื่อข้อมูลโดเมนนี้จากเอกสารเก่า ให้ `grep` โค้ดปัจจุบันเช็คซ้ำเสมอ อย่าเชื่อแค่ชื่อไฟล์เอกสารหรือความจำเก่า

| Entity | ความหมาย | เก็บที่ไหน (ทุกที่) | เขียนที่ | อ่านที่ | จุดสังเกต/ความเสี่ยง |
|---|---|---|---|---|---|
| ~~Relay flag / Relay leg record~~ (ลบแล้ว) | ระบบไม้เดิม — ดูคำเตือนด้านบน | ถูกลบทิ้งทั้งหมด | — | — | แทนที่ด้วย joint co-ownership (ยังไม่ได้แมปในเอกสารนี้) |
