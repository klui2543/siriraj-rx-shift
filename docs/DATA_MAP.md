...
### 2.4 ~~Relay legs~~ → Joint co-ownership (เปลี่ยนระบบแล้ว) + ระบบชื่อเล่น

> ⚠️ **ล้าสมัย — อัปเดต 2026-07-13:** ระบบ "ไม้" (relay legs) ที่เคยแมปไว้ในตารางนี้**ถูกลบทิ้งทั้งหมดแล้ว** (commit `95753bb` "remove: ลบระบบไม้ (relay legs) ทั้งหมด — hard removal", ยืนยันแล้วว่า `_relayEnabled` เหลือ 0 จุดในโค้ดปัจจุบัน) แทนที่ด้วยระบบใหม่ **"ต่อเวรแบบเจ้าของร่วม" (joint co-ownership)** ซึ่งพัฒนาต่อไปถึง v2.0 แล้ว (`_combinedJointMap`, `joint`/`joint_note`/`joint_leave` records ผ่าน `refActionId` — พบ 22 จุดในโค้ดปัจจุบัน) หลักการต่างจากไม้เดิมโดยสิ้นเชิง: เวรยังเป็น **1 หน่วยเดียว** + มีเจ้าของร่วมได้หลายคน + โน้ต, **ไม่มี** strike/ghost/chain แบบเก่า **ตารางด้านล่าง (relay leg record) เป็นข้อมูลประวัติศาสตร์ ไม่ตรงกับโค้ดปัจจุบันแล้ว** — โดเมนนี้ยังไม่ถูกแมปใหม่ตามระบบ joint (รอทำรอบหน้าถ้าต้องการ) ส่วนแถวชื่อเล่นด้านล่างไม่ได้รับผลกระทบ ยังตรงกับโค้ดปัจจุบัน
